...
const main = async () => {
  ...
  const createdNft = request.moveCall({
  ...
  request.transferObjects([createdNft], request.pure.address(suiAddress));
  ...
  const response = await suiClient.signAndExecuteTransaction({ transaction: request, signer: keypair });
