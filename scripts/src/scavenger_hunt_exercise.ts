...
const pub_key = keypair.getPublicKey().toSuiAddress();
...
const main = async () => {
  ...
  tx.transferObjects([sui_coin], tx.pure.address(pub_key));
  ...
  const response = await suiClient.signAndExecuteTransaction({ transaction: tx, signer: keypair });
